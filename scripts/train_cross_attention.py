"""Training script for Cross-Attention DTI model."""
...
from src.models import DTIProfilerCrossAttention
...
def main(config_path: str = "config.yaml", protein_feat: str = None, mol_feat: str = None):
    ...
    if protein_feat is None or mol_feat is None:
        raise ValueError("Please provide paths to protein and molecule feature files using --protein_feat and --mol_feat")
    ...
    print("="*60)
    model = DTIProfilerCrossAttention(
        drug_dim=drug_dim,
    ...
    save_path = results_dir / f"cross_attention_{config['model']['protein_encoder']}_{config['model']['molecule_encoder']}.pth"
    ...
    history_df.to_csv(results_dir / f"history_cross_attention.csv", index=False)
    ...
    val_df.to_csv(results_dir / "val_predictions_cross_attention.csv")
    ...
    test_unseen_prot_df.to_csv(results_dir / "test_unseen_protein_predictions_cross_attention.csv")
    ...
    test_unseen_lig_df.to_csv(results_dir / "test_unseen_ligand_predictions_cross_attention.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Cross-Attention DTI model")
    parser.add_argument("--config", type=str, default="config.yaml", help="Path to config file")
    parser.add_argument("--protein_feat", type=str, required=True, help="Path to protein features h5 file")
    parser.add_argument("--mol_feat", type=str, required=True, help="Path to molecule features h5 file")
