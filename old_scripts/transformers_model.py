        ...
        for mol in molecule:
            ...
            class DTIProfilerTransformer(nn.Module):
                def __init__(self, latent_activation=nn.ReLU):
                    ...
                    transformer_layer = nn.TransformerEncoderLayer(d_model=1024, nhead=4, dim_feedforward=1024 * 4, dropout=0.1, batch_first=True)
                    self.transformer_encoder = nn.TransformerEncoder(transformer_layer, num_layers=2)
                    self.mlp = nn.Sequential(nn.Linear(1024, 256), nn.ReLU(), nn.Dropout(0.1), nn.Linear(256, 1), nn.Sigmoid())
                def forward(self, drug, target):
                    ...
                    logits = self.mlp(transformer_output_squeezed) # (batch_size, 1)
                    return logits
            ...
            model = DTIProfilerTransformer().to(DEVICE)
            ...
            for epoch in range(EPOCHS):
                ...
                if val_auc > best_val_auc: # Or use val_loss < best_val_loss
                    ...
                    torch.save(model.state_dict(), 'best_dti_model_transformers_{}_{}_{}.pth'.format(prot, mol, i))
                    print(f"  New best model saved with Val AUC: {best_val_auc:.3f}")

            print("Training finished.")

            model.load_state_dict(torch.load('best_dti_model_transformers_{}_{}_{}.pth'.format(prot, mol, i)))
            ...
            val_df[['UniProt', 'SMILES', 'Label', 'Predictions']].to_csv('val_pred_transformers_{}_{}_{}.csv'.format(prot, mol, i))
            ...
            test_df_unseen_prot[['UniProt', 'SMILES', 'Label', 'Predictions']].to_csv('test_pred_unseen_prot_transformers_{}_{}_{}.csv'.format(prot, mol, i))
            ...
            test_df_unseen_lig[['UniProt', 'SMILES', 'Label', 'Predictions']].to_csv('test_pred_unseen_lig_transformers_{}_{}_{}.csv'.format(prot, mol, i))
