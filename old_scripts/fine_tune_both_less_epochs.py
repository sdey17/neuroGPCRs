...
for i in range (5):
    ...
    if __name__ == "__main__":
        ...
        model = DTIProfilerWithFineTuning(
            d_model=512,
            n_heads=4,
            freeze_protein_encoder=False,  # SET TO TRUE TO FREEZE
            freeze_mol_encoder=False       # SET TO TRUE TO FREEZE
        ).to(DEVICE)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_auc > best_val_auc:
                ...
                torch.save(model.state_dict(), 'best_dti_end_to_end_unfrozen_both_ProtBert_MolFormer_{}.pth'.format(i))
                print(f"*** New best model saved! Val AUC: {best_val_auc:.4f} ***")
        
        print(f"\nTraining completed! Best Val AUC: {best_val_auc:.4f}")

        model.load_state_dict(torch.load('best_dti_end_to_end_unfrozen_both_ProtBert_MolFormer_{}.pth'.format(i)))
        ...
        val_df.to_csv('val_pred_fine_tune_both_ProtBert_MolFormer_{}.csv'.format(i))
        ...
        test_df_unseen_prot.to_csv('test_pred_unseen_prot_fine_tune_both_ProtBert_MolFormer_{}.csv'.format(i))
        ...
        test_df_unseen_lig.to_csv('test_pred_unseen_lig_fine_tune_both_ProtBert_MolFormer_{}.csv'.format(i))
